...
<body>
    <div class="payroll-container">
        ...
        <table class="data-header-table">
            <tr>
                <td class="company-info">
                    ...
                    <p>NIT: {{ $desprendible['empresa']['nit'] ?? '000.000.000-0' }}</p>
                    <p>Dirección: {{ $desprendible['empresa']['direccion'] ?? 'Carrera 1 # 1-1, Ciudad' }}</p>
                    <p>Periodo de Pago: {{ $desprendible['periodo']['inicio'] ?? 'DD/MM/AAAA' }} 
                        al {{ $desprendible['periodo']['fin'] ?? 'DD/MM/AAAA' }}</p>
                </td>
                <td class="employee-info">
                    <p><strong class="label">Nombres:</strong> <span class="value">{{ $desprendible['empleado']['nombres'] ?? 'N/A' }}</span></p>
                    <p><strong class="label">Cédula:</strong> <span class="value">{{ $desprendible['empleado']['cedula'] ?? 'N/A' }}</span></p>
                    <p><strong class="label">Cargo:</strong> <span class="value">{{ $desprendible['empleado']['cargo'] ?? 'N/A' }}</span></p>
                    <p><strong class="label">Salario Base:</strong> 
                        <span class="value">${{ number_format($desprendible['empleado']['salario_base'] ?? 0, 0, ',', '.') }}</span></p>
                    <p><strong class="label">Banco / Cuenta:</strong> 
                        <span class="value">{{ $desprendible['empleado']['banco'] ?? 'N/A' }} / {{ $desprendible['empleado']['cuenta'] ?? 'N/A' }}</span></p>
                </td>
            </tr>
        </table>
        ...
        <table class="data-table">
            <thead>
                <tr><th class="concept">Concepto</th><th>Valor</th></tr>
            </thead>
            <tbody>
            ...
            <tr>
                <td>
                    <table class="neto-a-pagar-box">
                        <tr>
                            <th>NETO A PAGAR</th>
                            <td class="neto-value">${{ number_format($totalDevengos - $totalDeducciones, 0, ',', '.') }}</td>
                        </tr>
                    </table>
                </td>
            </tr>
        ...
        <div class="signature-section">
            <p class="disclaimer">
                Recibí la cantidad líquida de la presente nómina y declaro haber recibido el salario correspondiente al período indicado.
            </p>
